...
final class MovieHTTPService: AppNetworkService {
...
//		let errorMapper = BidsHTTPServiceErrorMapper()

		service = .init(config: serviceConfig, networkAdaptor: networkAdaptor, networkConnector: serviceConnector, errorMapper: nil)
	}
...
//			if allowLoginRetry, case BidsNetworkErrorResponse.info(let errorInfo) = error, errorInfo.code == BidsErrorCodes.invalidLogin {
