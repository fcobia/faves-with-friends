...
final class PreviewMovieNetworkManager: MovieNetworkManager {
	...
	static var mockMultiSearch: MovieDBSearchResults {
		try! MovieDBConstnts.movieDBJSONDecoder.decode(MultiSearchResults.self, from: multiSearchJSON.data(using: .utf8)!)
	}
	
	static var mockMovieDetails: Movie {
		try! MovieDBConstnts.movieDBJSONDecoder.decode(Movie.self, from: movieDetailJSON.data(using: .utf8)!)
	}
}
